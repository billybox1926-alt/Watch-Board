import { warLayerCards } from "@/data/seedData";
import { StatusBadge } from "@/components/StatusBadge";
import { IntelCardShell } from "@/components/IntelCardShell";
import type { IntelCard } from "@/data/types";

interface WarLayerTabProps {
  searchQuery: string;
  onSelectCard: (card: IntelCard) => void;
}

export function WarLayerTab({ searchQuery, onSelectCard }: WarLayerTabProps) {
  const q = searchQuery.toLowerCase();
  const filtered = warLayerCards.filter(c => c.title.toLowerCase().includes(q) || c.summary.toLowerCase().includes(q) || c.interpretation?.toLowerCase().includes(q));
